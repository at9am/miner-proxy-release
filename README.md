...
> 🔗 **作者公开测试节点**:`stratum+tcp://mpp.stargrain.net:3333`,可直接连上试跑。该节点对外收取 **3.8% 服务费**(作者维护费,已在面板公开),与上表「自建部署默认 0.3% 底费」相互独立 —— 自己部署仍是 0.3% 起。
...
Compose 固定使用 Linux host 网络模式，不配置 `ports` 端口映射。`server.listen` 和 `server.web_listen` 会直接监听宿主机地址；启动前必须确认 `3333`、`18080` 未被占用，并通过宿主机防火墙限制访问范围。
...
假设 `miner-proxy` 部署在 `192.168.1.10`，矿机软件填写：

```text
矿池地址：stratum+tcp://192.168.1.10:3333
钱包地址：你的PRL钱包地址
矿机名称：Rig01
```
...
```powershell
bzminer.exe -a pearl -p stratum+tcp://192.168.1.10:3333 -w 你的PRL钱包地址/Rig01
```

启动矿机后，确认日志中已经连接 TCP `3333`，并出现难度、算力或 accepted share。

> Stratum 挖矿协议为**纯 TCP**,只需放行 **TCP 3333**(矿机接入)与 **TCP 18080**(Web 面板),**无需开放 UDP 端口**。
